...
- **`origin/dev` PUSHES = FAZAL'S EXPLICIT REQUEST ONLY (STANDING, refined Fazal 2026-06-27).** Every push to `origin/dev` triggers the Vercel native auto-build + Railway redeploy = cost. So do NOT `git push origin dev` per change/merge, AND **do NOT push on a Cowork checkpoint/gate alone** — wait for **Fazal's explicit "push"** (Cowork relays his word). After a change passes its gate + local green, **integrate it into LOCAL `dev` (commit) and HOLD the push**. Mechanic on Fazal's push word = local-integrate the cleared branches + ONE `git push origin dev` (NOT N `gh pr merge`s). Per-change verification/gate/canary is UNCHANGED — only the PUSH waits for Fazal. Feature-branch pushes (for PRs/fixes) are fine (no dev build). `main` unchanged (Fazal-only). See `.viabe/cc-startup-protocol.md`.
