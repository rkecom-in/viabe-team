...
def test_mint_round_trip(monkeypatch):
    ...
    token, jti = mint_trial_end_token("tenant-abc", "standard")
...
def test_mint_unique_jti(monkeypatch):
    monkeypatch.setenv("OWNER_JWT_SECRET", _SECRET)
    _, jti1 = mint_trial_end_token("t", "standard")
    _, jti2 = mint_trial_end_token("t", "standard")
...
def test_mint_requires_secret(monkeypatch):
    ...
    with pytest.raises(TrialEndSecretMissing):
        mint_trial_end_token("t", "standard")  # fail closed — never mint a blank-key token
...
def test_audience_isolation(monkeypatch):
    ...
    monkeypatch.setenv("OWNER_JWT_SECRET", _SECRET)
    token, _ = mint_trial_end_token("t", "standard")
...
def test_deep_link_builder(monkeypatch):
    monkeypatch.setenv("OWNER_JWT_SECRET", _SECRET)
    token, _ = mint_trial_end_token("t", "founding")
