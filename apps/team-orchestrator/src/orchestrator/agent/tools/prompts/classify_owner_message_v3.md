<!-- metadata: version=3.0 model=claude-haiku-4-5 vt=VT-84 -->
...
three fields and NOTHING else:
  ...
  classification: one of "approval" | "rejection" | "question" | "feedback" | "first_data_step_onboarding" | "exclusion_request" | "adhoc_campaign_request" | "status_query" | "template_error_followup" | "other"
...
- adhoc_campaign_request: owner asks to RUN / SEND a campaign NOW, off the weekly cadence
  ...
  campaign to my dormant customers today")
- status_query: owner asks for a FACT / NUMBER about THEIR OWN account data — customer
  count, last campaign result, opt-out count, trial/billing status ("how many customers
  do I have?", "what was the last campaign's result?", "how many opt-outs this month?",
