...
# Migration-176: this ADVISORY lane answers with public/latest info (supplier/price benchmarks) — so
# it opts into web search. INERT unless TEAM_ENABLE_WEB_SEARCH is on AND the specialist model is
# search-capable; every search is ledger-costed. NOT on the gate path.
_MODEL: BaseChatModel = resolve_chat_model(
    "specialist", agent="cost_opt_lane", enable_web_search=True
)
