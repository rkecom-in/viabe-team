...
**VERIFY-THEN-CREATE OWNER-SIGNUP E2E IS LIVE ON DEV — Fazal to run the Sundaram e2e (dev HEAD `5e82d37`).** The "get everything in place before we restart the e2e" foundation is COMPLETE. Shipped this cycle (all merged to dev): VT-404 welcome reply-invite (#478), VT-405 A+B VTR discovery panel + per-field confirm (#476/#477), VT-406 A+B+reconciliation entity-match verify-gated signup + anchor (#479/#482/#484), VT-407 + minors widen Sandbox GST parse + address polish (#480/#487), **VT-408 hard GST gate / verify-then-create (#481, CL-442)**, VT-409 Sandbox auth-token fix — the GSTIN-search 500 was our top-vs-nested token bug (#483), VT-387 idempotency retry-window fix (#488), VT-388 `book_stationery` taxonomy (#485), CI flake quarantine (#486). **PARKED:** VT-386 (PII redaction registry, plan-first at Cowork gate); taxonomy 4 extra types (tailor/florist/meat_fish/optician — Fazal k-anon ruling pending); VT-410 (`send_whatsapp_message` sibling idempotency bug, Queued). Customer messaging FAIL-CLOSED (3 structural stops, CL-434). Externals unchanged: Meta F1 Meta-APPROVED (CL-438) + counsel C1–C3 (CL-439, the only gate to real customer sends) + VT-231 Mumbai before `dev→main`. Full state: `latest-snapshot.md`. *(Prior blocks — Sprint-7/8 closes, the 2026-06-12 Run-Control-Panel A–D batch CL-435/436 — all MERGED + superseded.)*
...
| **CL-421** | **2026-05-29** | **integration-agent, connectors, oauth, zero-manual-paste, vt-212, vt-222, sheet-redesign, apps-script-deprecated, customer-shape** | **ALL INTEGRATION-AGENT CONNECTORS MUST BE ZERO-MANUAL-PASTE AFTER OAUTH. No Apps Script paste, no copy-paste secrets, no developer-shaped setup steps. OAuth grant + auto-configuration via vendor API is the only acceptable flow. VT-212 walk surfaced Apps Script as customer-hostile (Tier-2/3 Indian SMB owner ≠ developer). Sheet connector pivots to Drive Push Notifications (Files.watch) primary + 10-min polling fallback (filed as VT-222, blocks VT-207). Shopify (VT-213) already conforms. `setup_push` + `apps_script_template` deprecated; VT-222 removes from happy path. **[CORRECTED by CL-427: Shopify conforms only once VT-283 OAuth managed-install ships.]** | **Fazal-issued LOCKED Standing** |
| **CL-422** | **2026-05-29** | **supabase, seoul, mumbai, region, residency, dev, prod, customer-data, dpdp, database, launch-blocker** | **Dev Supabase Seoul-accepted (dev/synthetic only); prod = Mumbai (VT-231 launch-blocker). HARD: no real customer data on dev until VT-231 closes. Don't re-flag Seoul as a DPDP issue.** | **Fazal Standing (launch-gate sunset)** |
| CL-423 | 2026-05-30 | pr, title, vt-id, numeric, allocator, ci-gate, pseudo-parent | Every PR title ends `(VT-<N>)` with a real allocator-claimed numeric row; pseudo-parents + dotted IDs rejected; `pr-title` CI gate enforces | Fazal Standing |
| CL-424 | 2026-05-30 | ultracode, fan-out, parallel, subagents, migration, allocator, vt-id, one-pr-per-row | xhigh ultracode for all CC tasks; fan-out guardrails: IDs + migration numbers allocated ONCE up-front (flock'd allocators, never hand-picked), one coherent PR per VT row, plan-first unchanged on risky rows | Fazal Standing |
| **CL-425 / VT-269** | **2026-06-01** | **dpdp, owner_inputs, sub-processor, consent, vision, privacy, lawful-basis** | **owner_inputs is a SUFFICIENT DPDP lawful basis for AI sub-processor transmission of customer PII (ledger images → vision); no separate consent flag; prod-enablement gated only by CL-422/VT-231; framing condition = VT-272 (counsel package)** | **Fazal Standing** |
| **CL-426** | **2026-06-01** | **vtr, human-on-the-loop, escalation, routing, pii, kg-injection, autonomy, de-identified, views, second-vtr, owner-authority** | **VTR = decaying human-on-the-loop; Fazal = VTR#1. Three-way routing LOCKED (agent acts · VTR resolves knowledge gaps · OWNER holds authority). Customer PII ENCRYPTED FROM VTR (de-identified views, DB-enforced since VT-360 + Gap-6). 2nd VTR requires assignment-scoped views + Devanagari validator-gap close FIRST.** | **Fazal Standing** |
| CL-427 | 2026-06-02 | connector, oauth, zero-paste, shopify, audit, owner-flow, client-credentials, managed-install | Connector-audit gate: every new connector passes "owner ONLY approves" BEFORE reaching owners; Shopify conforms only once VT-283 ships (corrects CL-421) | Fazal Standing |
| CL-428 | 2026-06-04 | alerts, trigger-kind, check-constraint, migration, drift, literal, tenant_alerts | `tenant_alerts.trigger_kind` DB CHECK stays synced to the Python `TriggerKind` Literal — any new kind = BOTH in the SAME PR (mig 089) | Cowork Standing |
| **CL-429** | **2026-06-06 (am. 06-09)** | **merge, self-merge, build, pillar-7, dev, green, gate, plan-first, scope** | **Merge-on-green: CC self-merges [BUILD] rows into `dev` when full-suite green + NO money/auth/PII/RLS/classifier/data-migration + scope matches + `merged` audit signal. Anything else → Cowork's gate. Plan-first only money/auth/PII/RLS. NEVER targets main (CL-432).** | **Fazal Standing (Pillar-7 amendment)** |
| CL-430 | 2026-06-06 | dashboard, sprint, artifact, pm-dashboard, retired | ONE dashboard: the sprint dashboard; PM dashboard RETIRED + tombstoned | Fazal Standing |
| **CL-431** | **2026-06-09** | **infra, env-vars, secrets, railway, vercel, supabase, prod, authorization, isolation, migration, injection, console** | **CC infra authority: DEV env vars autonomous; EVERY PROD env-var change = explicit Fazal auth first. Secrets never read/echoed into CC context (by-reference subshell; prod creds CONSUMED via railway-run injection). Prod migration runs Fazal-authorized. Explicit `--environment` always; one env per shell; `apply_migrations.py --expected-env` guard (VT-362).** | **Fazal Standing** |
| **CL-432** | **2026-06-09** | **branch, dev, main, promotion, staging, deploy, prod, pillar-7, railway, vercel, vt-363** | **Two branches: `dev` (staging stack) = CC's default PR base; `main` (prod/Mumbai) = Fazal-authorized promotion ONLY; main merge never auto-ships to prod (manual-gated).** | **Fazal Standing** |
| **CL-433** | **2026-06-09** | **trial, billing, refund, subscribe, lapsed, razorpay, gstin, money, state-machine, templates, whatsapp, card, auto-charge** | **30-day free trial, NO card in trial, opt-in subscribe (NO auto-charge), `lapsed` re-subscribable, NO refund ever. Refund subsystem DELETED (don't resurrect). `gstin_verified` gate on `subscribe`.** | **Fazal Standing (money)** |
| CL-434 | 2026-06-11 | agents, send, choke-point, opt-out, dsr, autonomy, kill-switch, grounding, validator, roadmap, journey, onboarding, rls, purge-order, fail-closed, consent, meta-sid, marketing | 6-gap standing surfaces: single send choke point `agent_send_draft`; THREE customer-send stops (no L3 caller / empty MARKETING_CONSENT_VERSIONS / null Meta SIDs — F1+C2 flip together + re-canary); opt-out/DSR BEFORE any reply-consuming gate; owner-confirmed drafts; frozen-bundle validator (text_hi + sentence-leading = pre-promotion); kill switches fail-CLOSED; new PII tables FORCE RLS + _PURGE_ORDER day one | Standing |
| **CL-435** | **2026-06-11** | **clau, roles, architecture, audit, vtr, run-control, panel, overrides, pause, fork, rerun, step, interceptor, canvas, n8n, rls, redaction, timeline** | **Clau RETIRED — Cowork owns architecture/sequencing/audit (adversarial gates). Run-Control Panel A–D granted (VT-374 substrate MERGED #458; VT-375 canvas MERGED #459; VT-376 controls; VT-377 multi-VTR capability-complete). Invariants: VTR I/O redacted/keys-only; re-runs never inherit approvals nor bypass send/opt-out/consent gates (manifest + import-raise + CI grep); control rows RLS+FORCE+purged+audited; opt-out/DSR pause-EXEMPT (I6); detect-and-escalate on rerun/approval overlap (C1-A). Batch CLOSED 2026-06-12 (CL-436).** | **Fazal Standing** |
| **CL-433** | **2026-06-09** | **trial, billing, refund, subscribe, lapsed, razorpay, gstin, money, state-machine** | **30-day free trial, NO card in trial, opt-in subscribe (NO auto-charge), `lapsed` re-subscribable, NO refund ever. Refund subsystem DELETED (don't resurrect). `gstin_verified` activation gate moved off `card_captured` onto `subscribe`.** | **Fazal Standing (money)** |
| **CL-437** | **2026-06-12** | **privacy, vtr, pii, redaction, error-column, outbox, retention, update-lock** | **Three privacy rulings: VT-379 error-column redaction granted; VTR-PII lock REAFFIRMED + extended to UPDATES (VTR cannot see OR update customer PII; UI-masking model REJECTED, don't re-propose); outbox retention policy.** | **Fazal Standing** |
| **CL-438** | **2026-06-12** | **whatsapp, templates, meta, f1, sids, autonomy, ask-act, l3, c2, opt-out, consent** | **F1 LANDED (10 SIDs EN+HI for the 5 agent-surface templates, Meta-APPROVED) + AUTONOMY-FIRST principle (Ask/Act model; repeated owner interruption = product bug). Non-bypassable Act floor = opt-out/DSR + DPDP consent (C2) + money-bearing always-confirm. C2 stays the structural stop on real customer sends until counsel.** | **Fazal Standing** |
| **CL-440** | **2026-06-13** | **positioning, brand, viabe-team, viabe-reports, location-feasibility-report, awareness, top-of-funnel** | **Viabe Team = the core product (autonomous AI business platform). Viabe Reports = an AWARENESS feature, not a co-equal product; externally always "Viabe's Location Feasibility Report", never a standalone product. Reports stays a technically separate codebase/DB/KG — "feature" is the market framing, not a repo merge. Carry every session.** | **Fazal Standing** |
| **CL-442** | **2026-06-24** | **gstin, hard-gate, signup, verify-then-create, trial, activation, dpdp, kyc, vendor-down, invalid-gstin, tenant-provision** | **GSTIN HARD-GATE MOVES FROM ACTIVATION TO SIGNUP. No tenant reaches trial OR active without `verification_status ∈ {gstin_verified, vtr_verified}`. Enforcement = verify-then-create: GSTIN verified server-side BEFORE the `tenants` row exists (nothing held for a rejected business). `vendor_down` HOLDS w/ retry (CAP-EXEMPT from the 5/day budget), `invalid_gstin` REJECTS (generic copy, no enumeration oracle). VT-406 "use my typed name / no-match" path no longer proceeds unverified; `tenant_provision` backdoor CLOSED for new/unknown numbers. VT-361 `subscribe` gate RETAINED as defense-in-depth. Build = VT-408 (#481); verify round-trip = VT-406. Supersedes the kyc-decision.md "Activation gate" framing.** | **Fazal Standing (money/KYC)** |
...
| 2026-06-24 | CC (overnight) | Regenerated from git-truth (dev HEAD `5e82d37`, reconciled vs `git log origin/dev` per Rule #14). Refreshed Phase-status block to the verify-then-create signup batch; added the missing recent Standing rows CL-433/437/438/440/442 to the active table (the file had drifted ~12 days, ending at CL-435 mid-Run-Control). Run-Control A–D batch (CL-435/436) marked CLOSED. |
...
- **CL-428** (2026-06-04, Cowork-issued, STANDING) — **`tenant_alerts.trigger_kind` DB CHECK must stay synced to the `TriggerKind` Literal in alerts/triggers.py.** mig 089 (VT-76) reconciled a VT-79 drift (3 kinds added to the code Literal — tenant_isolation_breach/dsr_rate_anomaly/pii_in_log — but never to the DB CHECK; would have crashed those detectors on first real fire). Discipline: any new trigger kind = BOTH the Literal AND a CHECK-extending migration in the SAME PR. Tags: alerts, triggers, db-check, vt-79, vt-76, migration, observability, schema-sync.
