...
def _seed_batch(
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        row = conn.execute(
            ...
            "owner_feedback) VALUES (%s, %s, %s, %s, %s) RETURNING id",
            (str(tenant), str(work_item), agent, status, owner_feedback),
        ).fetchone()
    assert row is not None
    return UUID(str(row[0]))
...
def test_sent_path_captures_exact_text_then_redacts_params(substrate):  # type: ignore[no-untyped-def]
    ...
    row = audit[0]
    assert row["message_sid"] == result.message_sid
...
@pytest.mark.parametrize("decision,expected_status", [("rejected", "rejected"),
                                                      ("timeout", "cancelled")])
def test_batch_terminal_resolve_redacts_owner_feedback(  # type: ignore[no-untyped-def]
    substrate, decision, expected_status,
):
    """Drive the REAL apply_agent_decision resolve path: rejected/cancelled are terminal
    batch closes -> owner_feedback redacted to its sha256 marker (a non-PII string), status
    kept."""
