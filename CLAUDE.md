...
- **BATCH PUSHES TO `origin/dev` (STANDING, Fazal 2026-06-24).** Every push to `origin/dev` triggers the CLI `--prebuilt` Vercel deploy + Railway redeploy = cost. So do NOT `git push origin dev` per change/merge. After a change passes its gate (Cowork gate for risk rows) + local green, **integrate it into LOCAL `dev` (commit) and HOLD the push**; push `origin/dev` in BATCHES only on (a) Cowork/Fazal "push" signal, or (b) a deploy/test checkpoint (e.g. before an e2e — dev only deploys on push). Mechanic = local-integrate the cleared branches + ONE `git push origin dev` (NOT N `gh pr merge`s). Per-change verification/gate/canary is UNCHANGED — only the PUSH is batched. Feature-branch pushes (for PRs/fixes) are fine (no dev build). `main` unchanged (Fazal-only). See `.viabe/cc-startup-protocol.md`.
