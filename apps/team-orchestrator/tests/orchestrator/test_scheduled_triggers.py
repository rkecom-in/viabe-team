...
def test_register_scheduled_triggers_idempotent(monkeypatch) -> None:
    ...
    # 6th (nightly L3 construction); VT-76 added the 7th (reconstitution sweep).
    assert first == 7, "expected 7 triggers registered on first call"
    assert second == 7, "second call must short-circuit (idempotent)"
    st._registered = False
