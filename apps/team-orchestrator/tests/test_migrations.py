...
def test_vt723_evidence_edge_stance_persists_and_refutation_never_counts(migrated):
    """Migration 202 + the ONE edge writer, executed against real Postgres.

    This test exists because of how the defect it guards escaped. PR #553 added stance by forking
    ``registry_seed._insert_source_edge``, and the fork's INSERT named ``supports`` and
    ``relevance`` — columns the schema did not have. Every test in that PR drove a
    ``RecordingConnection`` that appends the query string and never executes it, so the mismatch
    was invisible until ``--execute`` aborted the whole load on its first row. A writer whose SQL
    no test ever runs is an untested writer regardless of how many assertions surround it.

    The second half pins the invariant the column exists for: a REFUTING edge must be excluded from
    corroboration counting. Counted as support, an authoritative contradiction would promote the
    very claim it demolishes — the card looks better-corroborated the harder it is refuted.
    """
    ...
    dsn = migrated["dsn"]
    with psycopg.connect(dsn, autocommit=True) as conn:
        card_id, _card_key = _insert_o8_card(conn)  # ships one SUPPORTING edge of its own
        refuting_source_id = conn.execute(
            "INSERT INTO knowledge_sources "
            "(canonical_url, publisher, source_class, content_hash, acquired_at, usage_rights, "
            " retention_class, tainted) "
            "VALUES (%s, 'VT-723 refuting authority', 't1', %s, now(), "
            " '{\"status\":\"public_domain\"}'::jsonb, 'lifecycle_managed', true) RETURNING id",
            (f"https://example.invalid/refutes/{uuid4()}", uuid4().hex + uuid4().hex),
        ).fetchone()[0]

        # The writer reads exactly two attributes off the card. A stand-in keeps this test about
        # the SQL/schema agreement — the thing that was never exercised — rather than about
        # KnowledgeCard validation, which unit tests already cover.
        card = SimpleNamespace(
            card_version_id=str(card_id), independence_cluster=f"cluster:{uuid4()}"
        )
        registry_seed._insert_source_edge(  # type: ignore[arg-type]
            conn, card, str(refuting_source_id), supports=False
        )

        row = conn.execute(
            "SELECT supports, independence_cluster_id FROM knowledge_card_sources "
            "WHERE card_id = %s AND source_id = %s",
            (card_id, refuting_source_id),
        ).fetchone()
        assert row is not None, "the real writer's real SQL must actually insert a row"
        assert row[0] is False, "a refuting source must persist as a refutation"
        assert row[1] == card.independence_cluster, "the edge must carry the cluster it was given"

        total, supporting = conn.execute(
            "SELECT count(*), count(*) FILTER (WHERE supports) "
            "FROM knowledge_card_sources WHERE card_id = %s",
            (card_id,),
        ).fetchone()
        assert total == 2, "two edges attached"
        assert supporting == 1, (
            "corroboration counting must see ONE supporting cluster, not two — this is the "
            "inversion migration 202 exists to prevent"
        )

        # Replay semantics, asserted rather than assumed: the edge is keyed (card_id, source_id)
        # ON CONFLICT DO NOTHING, so a re-run cannot duplicate the edge AND cannot silently
        # overwrite a recorded refutation with the default TRUE. Correcting a stance is therefore
        # a deliberate UPDATE, never a side effect of re-running a load.
        registry_seed._insert_source_edge(  # type: ignore[arg-type]
            conn, card, str(refuting_source_id), supports=True
        )
        replayed = conn.execute(
            "SELECT count(*), bool_or(supports) FROM knowledge_card_sources "
            "WHERE card_id = %s AND source_id = %s",
            (card_id, refuting_source_id),
        ).fetchone()
